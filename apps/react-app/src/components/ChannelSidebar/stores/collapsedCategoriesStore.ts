...
export const useCollapsedCategoriesStore = create(
  persist(
    combine({ collapsedCategories: {} as { [id: string]: boolean | undefined } }, (set, get) => ({
      acions: {
        toggleCategory: (id: string) =>
          set((s) => {
            if (!s.collapsedCategories[id]) {
              return { collapsedCategories: { ...s.collapsedCategories, [id]: true } };
            }
...
export const collapsedCategoriesActions = useCollapsedCategoriesStore.getState().acions;
