...
const dropdownMenuItemStyles = cva(
  'flex items-center justify-between rounded focus:outline-none cursor-pointer text-sm font-medium whitespace-nowrap font shrink-0 px-2 py-1.5 text-white',
  {
    variants: {
      intent: {
...
export const DropdownMenuItem = forwardRef<HTMLDivElement, DropdownMenuItemCVAProps>(
  (
    { isActive = false, intent = 'primary', fullWidth = false, className = '', children, ...props },
    ref,
  ) => (
    <Item
