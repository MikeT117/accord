...
const contextMenuItemStyles = cva(
  'flex items-center justify-between rounded focus:outline-none cursor-pointer text-sm whitespace-nowrap font shrink-0 px-2 py-1.5 text-white',
  {
    variants: {
      intent: {
...
export const ContextMenuItem = forwardRef<HTMLDivElement, ContextMenuItemCVAProps>(
  ({ className = '', fullWidth = false, intent = 'primary', isActive = false, ...props }, ref) => {
    return (
      <Item
