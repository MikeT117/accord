...
WITH guilds_cte AS (
	SELECT
    g.id, g.name, g.description, g.is_discoverable, g.channel_count, g.member_count, g.creator_id, g.guild_category_id, g.updated_at
	FROM
    guilds g
	INNER JOIN
...
channels_cte AS (
	SELECT
    c.id, c.name, c.topic, c.channel_type, c.parent_id, c.creator_id, c.guild_id, c.updated_at
	FROM
    channels c
	INNER JOIN
