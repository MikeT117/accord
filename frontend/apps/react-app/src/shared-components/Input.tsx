...
>(({ leftInputElement, rightInputElement, className = '', isError = false, ...props }, ref) => {
  return (
    <div
      className={`flex w-full items-center rounded-md bg-grayA-3 px-3 py-0 focus-within:ring-2 ${
    ...
    >
      ...
      <input
        ref={ref}
        type='text'
        className='w-full border-none bg-transparent px-0 py-2.5 text-sm text-gray-11 focus:text-gray-12 focus:ring-0'
        {...props}
      />
      {rightInputElement && (
